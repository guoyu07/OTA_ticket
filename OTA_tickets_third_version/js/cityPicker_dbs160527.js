define('cityPicker_dbs160527', ['jquery'], function ($) {
...
return function(){
    ...
    /* 城市HTML模板 */
    Vcity._template = [
        '<p class="tip_dbs">热门城市(支持汉字/拼音)</p>',
        '<ul>',
        '<li class="on_dbs">热门城市</li>',
        '<li>ABCDEFGH</li>',
        '<li>IJKLMNOP</li>',
        '<li>QRSTUVWXYZ</li>',
        '</ul>'
    ...
    Vcity.CitySelector.prototype = {
        ...
        createWarp:function(){
            ...
            Vcity._m.on(document, 'click', function (event) {
                ...
                if (that.cityBox)Vcity._m.addClass('hide_dbs', that.cityBox);
                if (that.ul)Vcity._m.addClass('hide_dbs', that.ul);
                if(that.myIframe)Vcity._m.addClass('hide_dbs',that.myIframe);
            });
            div.className = 'citySelector_dbs';
            ...
            childdiv.className = 'cityBox_dbs';
            childdiv.id = 'cityBox_dbs';
            childdiv.innerHTML = Vcity._template.join('');
            var hotCity = this.hotCity =  document.createElement('div');
            hotCity.className = 'hotCity_dbs';
        ...
        createHotCity:function(){
            ...
            for(key in oCity){
                ...
                // 先设置全部隐藏hide
                odiv.className = key + ' ' + 'cityTab_dbs hide_dbs';
                sortKey=[];
                ...
                // 移除热门城市的隐藏CSS
                Vcity._m.removeClass('hide_dbs',this.hot);
        ...
        tabChange:function(){
            ...
            for(var i=0,n=lis.length;i<n;i++){
                ...
                lis[i].onclick = function(){
                    for(var j=0;j<n;j++){
                        Vcity._m.removeClass('on_dbs',lis[j]);
                        Vcity._m.addClass('hide_dbs',divs[j]);
                    }
                    Vcity._m.addClass('on_dbs',this);
                    Vcity._m.removeClass('hide_dbs',divs[this.index]);
        ...
        linkEvent:function(){
            ...
            for(var i=0,n=links.length;i<n;i++){
                links[i].onclick = function(){
                    ...
                    Vcity._m.addClass('hide_dbs',that.cityBox);
                    /* 点击城市名的时候隐藏myIframe */
                    Vcity._m.addClass('hide_dbs',that.myIframe);
                }
            }
        },
        ...
        inputEvent:function(){
            ...
            Vcity._m.on(this.input,'click',function(event){
                ...
                if(!that.cityBox){
                    that.createWarp();
                }else if(!!that.cityBox && Vcity._m.hasClass('hide_dbs',that.cityBox)){
                    // slideul 不存在或者 slideul存在但是是隐藏的时候 两者不能共存
                    if(!that.ul || (that.ul && Vcity._m.hasClass('hide_dbs',that.ul))){
                        Vcity._m.removeClass('hide_dbs',that.cityBox);

                        /* IE6 移除iframe 的hide 样式 */
                        //alert('click');
                        Vcity._m.removeClass('hide_dbs',that.myIframe);
                        that.changeIframe();
                    }
                }
            });
            ...
            Vcity._m.on(this.input,'keyup',function(event){
                ...
                // 下拉菜单显示的时候捕捉按键事件
                if(that.ul && !Vcity._m.hasClass('hide_dbs',that.ul) && !that.isEmpty){
        ...
        createUl:function () {
            ...
            if (value !== '') {
                ...
                for (var i = 0, n = Vcity.allCity.length; i < n; i++) {
                    if (reg.test(Vcity.allCity[i])) {
                        ...
                        if (searchResult.length !== 0) {
                            str = '<li><b class="cityname_dbs">' + match[1] + '</b><b class="cityspell_dbs">' + match[2] + '</b></li>';
                        } else {
                            str = '<li class="on_dbs"><b class="cityname_dbs">' + match[1] + '</b><b class="cityspell_dbs">' + match[2] + '</b></li>';
                        }
                ...
                if (searchResult.length == 0) {
                    this.isEmpty = true;
                    str = '<li class="empty_dbs">对不起，没有找到数据 "<em>' + value + '</em>"</li>';
                    searchResult.push(str);
                }
                // 如果slideul不存在则添加ul
                if (!this.ul) {
                    var ul = this.ul = document.createElement('ul');
                    ul.className = 'cityslide_dbs';
                    this.rootDiv && this.rootDiv.appendChild(ul);
                    // 记录按键次数，方向键
                    this.count = 0;
                } else if (this.ul && Vcity._m.hasClass('hide_dbs', this.ul)) {
                    this.count = 0;
                    Vcity._m.removeClass('hide_dbs', this.ul);
                }
                ...
                Vcity._m.addClass('hide_dbs',this.ul);
                Vcity._m.removeClass('hide_dbs',this.cityBox);

                Vcity._m.removeClass('hide_dbs',this.myIframe);
        ...
        KeyboardEvent:function(event,keycode){
            ...
            switch(keycode){
                case 40: //向下箭头↓
                    ...
                    for(var i=0;i<len;i++){
                        Vcity._m.removeClass('on_dbs',lis[i]);
                    }
                    Vcity._m.addClass('on_dbs',lis[this.count]);
                    break;
                case 38: //向上箭头↑
                    this.count--;
                ...
                case 13: // enter键
                    ...
                    Vcity._m.addClass('hide_dbs',this.ul);
                    /* IE6 */
                    Vcity._m.addClass('hide_dbs',this.myIframe);
                    break;
                default:
                    break;
            }
        ...
        liEvent:function(){
            ...
            for(var i = 0,n = lis.length;i < n;i++){
                Vcity._m.on(lis[i],'click',function(event){
                    ...
                    $(that.input).change();
                    Vcity._m.addClass('hide_dbs',that.ul);
                    /* IE6 下拉菜单点击事件 */
                    Vcity._m.addClass('hide_dbs',that.myIframe);
                });
                Vcity._m.on(lis[i],'mouseover',function(event){
                    event = Vcity._m.getEvent(event);
                    var target = Vcity._m.getTarget(event);
                    Vcity._m.addClass('on_dbs',target);
                });
                Vcity._m.on(lis[i],'mouseout',function(event){
                    event = Vcity._m.getEvent(event);
                    var target = Vcity._m.getTarget(event);
                    Vcity._m.removeClass('on_dbs',target);
                })
            }
        }
    };
        ...
        var test=new Vcity.CitySelector({input:'city_input1_dbs2016'});
        
        $("#city_input1_img_dbs2016").click(function(event){  

             //var test=new Vcity.CitySelector({input:'city_input1_dbs2016'});
             Vcity._m.stopPropagation(event);
            // test.initialize({input:'city_input1_dbs2016'});
            $("#city_input1_dbs2016").trigger("click");
             //test.createWarp();           
        });


        var test=new Vcity.CitySelector({input:'city_input2_dbs2016'});

          $("#city_input2_img_dbs2016").click(function(event){
           //  var test=new Vcity.CitySelector({input:'city_input2_dbs2016'});
             Vcity._m.stopPropagation(event);
             $("#city_input2_dbs2016").trigger("click");
           //  test.initialize({input:'city_input2_dbs2016'});
            // test.createWarp();             
            });

        var test=new Vcity.CitySelector({input:'city_input3_dbs2016'});
           $("#city_input3_img_dbs2016").click(function(event){
            // var test=new Vcity.CitySelector({input:'city_input3_dbs2016'});
             Vcity._m.stopPropagation(event);
             $("#city_input3_dbs2016").trigger("click");
            // test.createWarp();   
           
            });

    
        var test=new Vcity.CitySelector({input:'city_input4_dbs2016'});

           $("#city_input4_img_dbs2016").click(function(event){
            // var test=new Vcity.CitySelector({input:'city_input4_dbs2016'});
             Vcity._m.stopPropagation(event);
             $("#city_input4_dbs2016").trigger("click");
            // test.createWarp();   
